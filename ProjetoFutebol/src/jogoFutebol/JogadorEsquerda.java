...
public class JogadorEsquerda extends Agent {

	int iterator = 0;
	public static int xJogador, yJogador;
	private static final int JOGADOR = 2;
	...
	protected void setup() {
		...
		System.out.println("Posicao do jogador esquerda (X= " + x + "  Y= " + y
				+ ") definida");
			...
			if (msg != null) {
				...
				if (mensagem.equals("Mover")) {
					...
					if (xJogador == Campo.xBola && yJogador == Campo.yBola) {
						...
						if(JogadorDireita.xJogador==xJogador && JogadorDireita.yJogador==yJogador){
						...
						if (num == 0) {
							if (Campo.permitido == true) {
								...
								ACLMessage msgMover = new ACLMessage(
										ACLMessage.INFORM);
				...
				if (mensagem.equals("Voltar")) {
					...
					System.out.println("Posicao do jogador esquerda (X= " + x
							+ "  Y= " + y + ") definida");
